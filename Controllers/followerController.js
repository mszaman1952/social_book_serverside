...
if (!followId || !following) { 
  res.status(404).json({  
    ...
    await userProfileModel.findByIdAndUpdate(followerId, { $push: { following: followingId } });
    await userProfileModel.findByIdAndUpdate(followingId, { $push: { followers: followerId } });
...
const unfollowUser = async (req, res) => {
  try {
    ...
    if (result.deletedCount > 0) {
      // Update the follower's and following user's arrays in the User model
      await userProfileModel.findByIdAndUpdate(followerId, { $pull: { following: followingId } });
      await userProfileModel.findByIdAndUpdate(followingId, { $pull: { followers: followerId } });

      res.json({ success: true, message: 'Unfollowed successfully' });
    } else {
      res.json({ success: false, message: 'Not following this user' });
    }
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal Server Error' });
