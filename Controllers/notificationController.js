...
const markAllNotificationsAsRead = async (req, res) => {
  try {
    ...
    if (notifications.length === 0) {
      return res.json({ success: false, message: 'No notifications found for the given user' });
    }
    ...
    if (allRead) {
      return res.json({ success: false, message: 'All notifications are already marked as read' });
    }
