...
const { forgetPassword_URL_sentemail } = require('../utilities/forgetPassword_URL_sentemail');
...
exports.activate_user = async(req)=>{
    try {
        ...
        await userProfileModel.updateOne({userId:verificationData.id},{$set:{userName:profileUserName,userId:verificationData.id}},{upsert: true});
...
exports.login_user = async(req)=>{
    try {
        ...
        if(!passwordCheck){
            return {status:"Fail",message:"invalid your email/password please try again."};
        };
...
exports.forget_password = async(req)=>{
    try {
        ...
        return {status:"success",message:"please check Your email and set your own new password."};
...
exports.set_password = async(req)=>{
    try {
        ...
        await userModel.findByIdAndUpdate({_id:verificationData.id},{$set:{password:passwordBcrypt}});

        return {status:"success",message:"new password set successfully."};
...
exports.create_profile = async(req)=>{
    try {
        ...
        const {userName,birthYear,job,bio,workplace,high_School,college,current_city,home_town,gender,relationship,instagram,twitter,telegram,discord,userBlockId,savePostId,pinPostId} = req.body;
        ...
        const  validateField = (value, minLength, maxLength, fieldName)=>{
            if (value && !validationLength(value, minLength, maxLength)) {
                return { status: "Fail", message:`${fieldName} must be between ${minLength} and ${maxLength} characters.`};
            }
            return null;
        };
        ...
        if(userBlockId||savePostId||pinPostId){
            await userProfileModel.updateOne({userId:req.userId},{$addToSet:{block_user:userBlockId,save_post:savePostId,pin_post:pinPostId}},{upsert: true});
        };

        await userProfileModel.updateOne({userId:req.userId},{$set:req.body,userImage:profile_img,userBannerImg:banner_img},{upsert: true});
...
exports.reset_profile = async(req)=>{
    try {
        ...
        await userProfileModel.updateOne({userId:req.userId},{
            $set:{
                userBannerImg:"",
                birthYear:"",
                job:"",
    ...
    return {status:"success",message:"profile reset successfully."};
...
exports.profile_block_pin_save = async(req)=>{
    try {
        ...
        const {unblock,unpin,unsave} = req.body;

        const userData = await userProfileModel.findOne({userId:req.userId}).select({'block_user':1,"pin_post":1,"save_post":1,"_id":0});

        if(unblock){
            const bloack_user = userData.block_user;
            const checkB = bloack_user.includes(unblock);
            if(!checkB){
               return {status:"Fail",message:"No user found for this id."};
            };
        };
        ...
        if(unpin){
            ...
            if(!checkP){
               return {status:"Fail",message:"No pin post found for this id."};
            };
        };
        ...
        if(unsave){
            ...
            if(!checkS){
               return {status:"Fail",message:"No save post found for this id."};
            };
        };

        await userProfileModel.findOneAndUpdate({userId:req.userId}, { $pull: { block_user: unblock,pin_post:unpin,save_post:unsave} });

        return {status:"success",message:"successfully work."};
...
exports.block_usre_get = async(req)=>{
    try {
        ...
        let userProject = {$project:{"block_user":1,"_id":0}};
        
        let categoryLookup = {$lookup:{from:"user-profiles",localField:"block_user",foreignField:"_id",as:"block_user"}};

        let blockuserProject = {$project:{"block_user.block_user":0,"block_user.pin_post":0,"block_user.save_post":0,"block_user.profile_lock":0,"block_user.createdAt":0,"block_user.userId":0,"block_user.updatedAt":0}};

        const blockUserDatfa = await userProfileModel.aggregate([matchUser,userProject,categoryLookup,blockuserProject]);

        return {status:"success",data:blockUserDatfa};
...
exports.save_post_get = async(req)=>{
    try {
        ...
        let userProject = {$project:{"save_post":1,"_id":0}};
        
        let categoryLookup = {$lookup:{from:"posts",localField:"save_post",foreignField:"_id",as:"save_post"}};

        let blockuserProject = {$project:{"block_user.block_user":0,"block_user.pin_post":0,"block_user.save_post":0,"block_user.profile_lock":0,"block_user.createdAt":0,"block_user.userId":0,"block_user.updatedAt":0}};

        const savePostData = await userProfileModel.aggregate([matchUser,userProject,categoryLookup,blockuserProject]);

        return {status:"success",data:savePostData};
...
exports.pin_post_get = async(req)=>{
    try {
        ...
        let userProject = {$project:{"pin_post":1,"_id":0}};
        
        let categoryLookup = {$lookup:{from:"posts",localField:"pin_post",foreignField:"_id",as:"pin_post"}};

        let blockuserProject = {$project:{"block_user.block_user":0,"block_user.pin_post":0,"block_user.save_post":0,"block_user.profile_lock":0,"block_user.createdAt":0,"block_user.userId":0,"block_user.updatedAt":0}};

        const pinPostData = await userProfileModel.aggregate([matchUser,userProject,categoryLookup,blockuserProject]);

        return {status:"success",data:pinPostData};
...
exports.viewSingleProfile = async(req)=>{
    try {
        ...
        const data = await userProfileModel.findOne({_id:id});

        const checkB = data.block_user.includes(userid);
        if(checkB){
            return {status:"Fail",message:"you can't see this profile because You are  block."};
        };
        ...
        if(!checkF && data.profile_lock == true){
           ...
           return {status:"success",data:profileObj};
        };

        const Pdata = await userProfileModel.findOne({_id:id}).select({"userId":0,"block_user":0,"pin_post":0,"createdAt":0,"save_post":0,"updatedAt":0});
